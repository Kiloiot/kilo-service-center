...
const buildNavItems = (
  ...
  { title: NAV_ITEMS.BASE_STATIONS, path: ROUTES.BASE_STATIONS, icon: <RouterIcon /> },
  { title: NAV_ITEMS.ENDPOINTS, path: ROUTES.ENDPOINTS, icon: <DeviceHubIcon /> },
  { title: NAV_ITEMS.BLUEPRINTS, path: ROUTES.BLUEPRINTS, icon: <BlueprintIcon /> },
  ...
  ...(isServerAdmin
    ? [{ title: NAV_ITEMS.API_KEYS, path: ROUTES.API_KEYS, icon: <VpnKeyIcon /> }]
    : []),
  // Organizations: server admin only, ECE only
  ...(isServerAdmin && showOrgs
    ? [{ title: NAV_ITEMS.ORGANIZATIONS, path: ROUTES.ORGANIZATIONS, icon: <BusinessIcon /> }]
    : []),
  { title: NAV_ITEMS.CERTIFICATES, path: ROUTES.CERTIFICATES, icon: <SecurityIcon /> },
];
...
const AppNavigation: React.FC<AppNavigationProps> = ({ mobileOpen, handleDrawerToggle }) => {
  ...
  const iconColor = isDark ? theme.palette.grey[100] : theme.palette.primary.main;
  ...
  const drawer = (
    ...
    >
      ...
      >
        <img src={logoPath} alt={LOGO.ALT} style={{ width: LOGO.WIDTH, height: 'auto' }} />
      </Toolbar>
      <Divider />
      ...
      <Box sx={{ px: 2, py: 1.5, borderTop: `1px solid ${theme.palette.divider}` }}>
        ...
        <Box sx={{ display: 'flex', gap: 1.5 }}>
          ...
          <MuiLink href={versionInfo?.licenseUrl} target="_blank" rel="noopener" variant="caption">
            {BRAND.LICENSE}
          </MuiLink>
        </Box>
      </Box>
