...
import type { BaseStationAPI, BaseStationDetailAPI, BaseStationUI } from '@api-types/api';
...
export function mapBaseStation(bs: BaseStationAPI): BaseStationUI {
  ...
  return {
    ...
    connectionType: isDetail
      ? ((bs.connectionType || bs.connection_type || 'bssci').toUpperCase() as 'BSSCI' | 'MQTT')
      : 'BSSCI',
    createdAt: bs.firstSeen || bs.first_seen || bs.created_at || '',
    lastSeen: bs.lastSeen || bs.last_seen || bs.last_seen_at || '',
    serviceCenterUrl: isDetail ? bs.serviceCenterUrl || bs.service_center_url || '' : '',
    latitude: bs.latitude,
    longitude: bs.longitude,
    altitude: bs.altitude,
    locationSource:
      (bs.locationSource as 'gps' | 'manual') ||
      ((bs as BaseStationDetailAPI).locationSource as 'gps' | 'manual' | undefined),
    locationUpdatedAt: (bs as BaseStationDetailAPI).locationUpdatedAt,
    certificateExpiryDate: isDetail ? (bs as BaseStationDetailAPI).tlsCertExpiresAt : undefined,
    ...
    uptimeSeconds: isDetail ? (bs as BaseStationDetailAPI).uptimeSeconds : undefined,
    temperatureCelsius: isDetail ? (bs as BaseStationDetailAPI).temperatureCelsius : undefined,
    ...
    lastStatusAt: isDetail ? (bs as BaseStationDetailAPI).lastStatusAt : undefined,
  };
}
 ...
 */
export function mapBaseStationList(baseStations: BaseStationAPI[]): BaseStationUI[] {
