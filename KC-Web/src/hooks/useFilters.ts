...
import type { BaseStationFiltersState, EndpointFiltersState, SortState } from '@contexts/filters';
...
export function useBaseStationFilters() {
  const { state, setFilter, setSearch, setSort, setPagination, resetScope } = useFilters();

  return useMemo(
    () => ({
      filters: state.baseStations,
      setSearch: (search: string) => setSearch('baseStations', search),
      setStatus: (status: string[]) => setFilter('baseStations', 'status', status),
    ...
    [state.baseStations, setFilter, setSearch, setSort, setPagination, resetScope]
  );
}
...
export function useEndpointFilters() {
  ...
  return useMemo(
    () => ({
      ...
      setAttachState: (attachState: string[]) => setFilter('endpoints', 'attachState', attachState),
      setBidirectional: (bidi: boolean | null) => setFilter('endpoints', 'bidirectional', bidi),
