...
func TestComplexSCOperations(t *testing.T) {
	testLogger := logger.NewNop()
	mockEvStore := &mockEventStore{}

	// Test each complex operation in both encodings
	for _, encoding := range []string{"json", "msgpack"} {
		t.Run("SendAttachPropagate_"+encoding, func(t *testing.T) {
			mockConn := &assemblyMockConn{encoding: encoding}
			mockConn.Reset()
			...
			server := bssci.NewTestServer(testLogger, mockStorage, mockEvStore, 1,
				sessionSvc, downlinkSvc, statusSvc, connectionSvc, broadcaster,
				queueSerializer, auditLogger, tenantResolver)
			...
			// Execute SendAttachPropagate with all required parameters
			...
			// Log actual fields for inspection
			t.Logf("SendAttachPropagate (%s) emitted fields: %v", encoding, getFieldNames(msg))
		})

		t.Run("SendDetachPropagate_"+encoding, func(t *testing.T) {
			mockConn := &assemblyMockConn{encoding: encoding}
			mockConn.Reset()
			...
			// Verify mandatory endpoint field
			assert.Contains(t, msg, "epEui", "Must include epEui")
		...
		t.Run("SendDLDataQueue_"+encoding, func(t *testing.T) {
			mockConn := &assemblyMockConn{encoding: encoding}
			mockConn.Reset()
			...
			// Execute SendDLDataQueue with minimal required fields
			...
			// Verify mandatory downlink fields
		...
		t.Run("SendDLDataQueue_CounterDependent_"+encoding, func(t *testing.T) {
			mockConn := &assemblyMockConn{encoding: encoding}
			mockConn.Reset()
			...
			// Execute SendDLDataQueue with cntDepend=true
		...
		t.Run("SendDLDataRevoke_"+encoding, func(t *testing.T) {
			mockConn := &assemblyMockConn{encoding: encoding}
			mockConn.Reset()
			...
			// Verify mandatory revoke fields
			assert.Contains(t, msg, "queId", "Must include queId")
			assert.Contains(t, msg, "epEui", "Must include epEui")
		...
		t.Run("SendDLRXStatusQuery_"+encoding, func(t *testing.T) {
			mockConn := &assemblyMockConn{encoding: encoding}
			mockConn.Reset()
		...
		t.Run("SendULDataTransmit_"+encoding, func(t *testing.T) {
			mockConn := &assemblyMockConn{encoding: encoding}
			mockConn.Reset()
			...
			// Execute SendULDataTransmit with minimal required fields
			epEui := bssci.TestEpEui01
			nwkSnKey := []byte{0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
				0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF}
			shAddr := uint16(0x1234)
			...
			// Verify opId in message is negative (SC-originated)
			var msgOpId int64 //nolint:revive // msgOpId is distinct from opId field
			switch v := msg["opId"].(type) {
			case int64:
				msgOpId = v
			case float64:
			...
			assert.Less(t, msgOpId, int64(0), "SC-originated opId must be negative")

			// Verify mandatory transmit fields
			assert.Contains(t, msg, "epEui", "Must include epEui")
			...
			// Log actual fields for inspection (serializer may omit optional format/profile)
			t.Logf("SendULDataTransmit (%s) emitted fields: %v", encoding, getFieldNames(msg))
		})

		t.Run("SendVMActivate_"+encoding, func(t *testing.T) {
			mockConn := &assemblyMockConn{encoding: encoding}
			mockConn.Reset()
		...
		t.Run("SendVMDeactivate_"+encoding, func(t *testing.T) {
			mockConn := &assemblyMockConn{encoding: encoding}
			mockConn.Reset()
		...
		t.Run("SendVMStatus_"+encoding, func(t *testing.T) {
			mockConn := &assemblyMockConn{encoding: encoding}
			mockConn.Reset()
		...
		t.Run("SendVMDownlinkData_"+encoding, func(t *testing.T) {
			mockConn := &assemblyMockConn{encoding: encoding}
			mockConn.Reset()
			...
			// Verify mandatory VM downlink fields
			assert.Contains(t, msg, "epEui", "Must include epEui")
			assert.Contains(t, msg, "macType", "Must include macType")
			assert.Contains(t, msg, "userData", "Must include userData")
